...
std::vector<ALLEGRO_VERTEX> TileMapWaterMesh::get_vertexes()
{
   return vertexes;
}
   ...
   for (int x=0; x<tile_map.get_width(); x++)
   {
      ...
      else
      {
         ...
         pillar = LabyrinthOfLore::Rendering::TileMapMeshPillarTexturer(
               tile_atlas,
               pillar,
               tile_type_definition.get_tile_index_for_front_and_back_texture()+__unused_offset_to_first_texture_index,
               tile_type_definition.get_tile_index_for_right_and_left_texture()+__unused_offset_to_first_texture_index,
               tile_type_definition.get_tile_index_for_top_texture()+__unused_offset_to_first_texture_index,
