#pragma once


#include <Tileo/TileAtlas.hpp>


namespace LabyrinthOfLore
{
   namespace Rendering
   {
      class TileMapMeshCubeTexturer
      {
      private:
         Tileo::TileAtlas* tile_atlas;
         int tile_index_for_front_and_back_texture;
         int tile_index_for_right_and_left_texture;
         int tile_index_for_top_texture;

      public:
         TileMapMeshCubeTexturer(Tileo::TileAtlas* tile_atlas=nullptr, int tile_index_for_front_and_back_texture=0, int tile_index_for_right_and_left_texture=0, int tile_index_for_top_texture=0);
         ~TileMapMeshCubeTexturer();


      void apply_textures();
      };
   }
}



